...
import { ClozeItemType, ItemComponentRole } from "@/components/survey-editor/components/types";
...
const ClozeEditor: React.FC<ClozeEditorProps> = (props) => {
    ...
    const clozeItems = (responseGroup.items.find(item => item.role === ItemComponentRole.Cloze) as ItemGroupComponent)?.items || [];
    ...
    const onAddItem = (keyOfSelectedItem: string) => {
        ...
        //console.log('keyOfSelectedItem:', keyOfSelectedItem);
        ...
        updateSurveyItemWithNewItems(newItems);
    };
